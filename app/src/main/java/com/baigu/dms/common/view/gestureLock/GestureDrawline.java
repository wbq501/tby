...
public class GestureDrawline extends View {
	...
	private List<GesturePointEntity> list;// 装有各个view坐标的集合
	private List<Pair<GesturePointEntity, GesturePointEntity>> lineList;// 记录画过的线
	private Map<String, GesturePointEntity> autoCheckPointMap;// 自动选中的情况点
	 ...
	 * 手指当前在哪个Point内
	 */
	private GesturePointEntity currentPoint;
	/**
	 * 用户绘图的回调
	 */
	...
	public GestureDrawline(Context context, List<GesturePointEntity> list, boolean isVerify,
						   String passWord, GestureCallBack callBack) {
		super(context);
		screenDispaly = ViewUtils.getScreenSize(context);
		...
		paint.setColor(Color.rgb(182, 205, 250));// 设置默认连线颜色
		paint.setAntiAlias(true);// 不显示锯齿

		this.list = list;
		this.lineList = new ArrayList<Pair<GesturePointEntity, GesturePointEntity>>();
	...
	private void initAutoCheckPointMap() {
		autoCheckPointMap = new HashMap<String,GesturePointEntity>();
	...
	private GesturePointEntity getGesturePointByNum(int num) {
		for (GesturePointEntity point : list) {
			if (point.getNum() == num) {
				return point;
			}
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				// 得到当前移动位置是处于哪个点内
				GesturePointEntity pointAt = getPointAt((int) event.getX(), (int) event.getY());
				// 代表当前用户手指处于点与点之前
				...
				if (pointAt == null || currentPoint.equals(pointAt) || Constants.POINT_STATE_SELECTED == pointAt.getPointState()) {
					...
					// 判断是否中间点需要选中
					GesturePointEntity betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
					if (betweenPoint != null && Constants.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
						// 存在中间点并且没有被选中
						Pair<GesturePointEntity, GesturePointEntity> pair1 = new Pair<GesturePointEntity, GesturePointEntity>(currentPoint, betweenPoint);
						lineList.add(pair1);
						passWordSb.append(betweenPoint.getNum());
						Pair<GesturePointEntity, GesturePointEntity> pair2 = new Pair<GesturePointEntity, GesturePointEntity>(betweenPoint, pointAt);
						lineList.add(pair2);
						...
						Pair<GesturePointEntity, GesturePointEntity> pair = new Pair<GesturePointEntity, GesturePointEntity>(currentPoint, pointAt);
						lineList.add(pair);
						passWordSb.append(pointAt.getNum());
						// 赋值当前的point;
						currentPoint = pointAt;
	 ...
	 */
	private GesturePointEntity getPointAt(int x, int y) {
	...
	private GesturePointEntity getBetweenCheckPoint(GesturePointEntity pointStart, GesturePointEntity pointEnd) {
	...
	private void clearScreenAndDrawList() {
		...
		for (Pair<GesturePointEntity, GesturePointEntity> pair : lineList) {
	...
	private void drawErrorPathTip() {
		canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
		paint.setColor(Color.rgb(238, 117, 110));// 设置默认线路颜色
		for (Pair<GesturePointEntity, GesturePointEntity> pair : lineList) {
